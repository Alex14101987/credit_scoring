...
def POS_CASH_balance_func(df: pd.DataFrame) -> pd.DataFrame:
    ...
    POS_CASH_balance['SK_ID_PREV'] = df_POS_CASH_balance['PosCashBalanceIDs'].str['SK_ID_PREV']
    POS_CASH_balance['SK_ID_CURR'] = df_POS_CASH_balance['PosCashBalanceIDs'].str['SK_ID_CURR']
    ...
    POS_CASH_balance['NAME_CONTRACT_STATUS'] = df_POS_CASH_balance['PosCashBalanceIDs'].str['NAME_CONTRACT_STATUS']
...
def bureau_func(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for n in range(len(df_bureau)):
        bureau['CREDIT_CURRENCY'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].CREDIT_CURRENCY
    bureau['CREDIT_CURRENCY'] = df_bureau['data.record.AmtCredit'].str['CREDIT_CURRENCY']
    ...
    for n in range(len(df_bureau)):
        bureau['AMT_CREDIT_MAX_OVERDUE'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].AMT_CREDIT_MAX_OVERDUE
    bureau['CNT_CREDIT_PROLONG'] = df_bureau['data.record.CNT_CREDIT_PROLONG']
    bureau["AMT_CREDIT_SUM"] = np.nan
    for n in range(len(df_bureau)):
        bureau['AMT_CREDIT_SUM'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].AMT_CREDIT_SUM
    bureau["AMT_CREDIT_SUM_DEBT"] = np.nan
    for n in range(len(df_bureau)):
        bureau['AMT_CREDIT_SUM_DEBT'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].AMT_CREDIT_SUM_DEBT
    bureau["AMT_CREDIT_SUM_LIMIT"] = np.nan
    for n in range(len(df_bureau)):
        bureau['AMT_CREDIT_SUM_LIMIT'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].AMT_CREDIT_SUM_LIMIT
    bureau["AMT_CREDIT_SUM_OVERDUE"] = np.nan
    for n in range(len(df_bureau)):
        bureau['AMT_CREDIT_SUM_OVERDUE'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].AMT_CREDIT_SUM_OVERDUE
    ...
    for n in range(len(df_bureau)):
        bureau['AMT_ANNUITY'].loc[n] = df_bureau['data.record.AmtCredit'].loc[n].AMT_ANNUITY
    print(type(bureau))
...
def main(path_log: str, path_bureau: str, path_POS_CASH_balance: str) -> pd.DataFrame:
    ...
    POS_CASH_balance_func(df).to_csv(path_POS_CASH_balance)
    bureau_func(df).to_csv(path_bureau)
